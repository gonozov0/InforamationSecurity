...
class User:
	...
	def start_authentication(self, usn, pas, scs = DEFAULT_SECRETSIZE):
		...
		self.username = usn
		self.password = pas
		self.a = _to_int(_randombytes(scs))
	...
	def process_challenge(self, slt, ver_B):
		...
		k = _to_int(_H(self.hasher(), self.N, _pad(self.g, l)))
		...
		t1 = ver_B - k * pow(self.g, x, self.N)
		t2 = self.a + u * x
...
class Verifier:
	...
	def get_challenge(self, usn, slt, ver, usr_A, scs = DEFAULT_SECRETSIZE):
		...
		# B = k*ver + g^b
		self.B = (k * ver + pow(self.g, self.b, self.N)) % self.N
